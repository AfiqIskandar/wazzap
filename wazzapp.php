...
class Person
{
    ...
    public function joinChannel($channelName) {
        $theChannelName=$channelName::$theChannelName;
        if($this->validateChanel($theChannelName)){
            array_push($this->theChannel,$theChannelName);
            $channelName->setMember($this->name);
        }
    }

    public function validateChanel($channel_name){
        return !in_array($channel_name,$this->theChannel);
    }
...
{
    // <YOUR CODE HERE>
    public static $theChannelName=[];
    public static $channels=[];
    protected $members=[];

    private function __construct($name){
        self::$theChannelName=$name;
    }
